...
async function seed() {
  try {
    ...
    for (const product of products) {
      await db.run(
        `INSERT INTO products (name, category, sku, unit_price, supplier, description) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [product.name, product.category, product.sku, product.unit_price, product.supplier, product.description]
      );
    }
    ...
    for (let i = 1; i <= products.length; i++) {
      ...
      await db.run(
        `INSERT INTO inventory (product_id, warehouse_id, quantity_on_hand, reorder_point) 
         VALUES (?, ?, ?, ?)`,
        [i, warehouseId, quantity, reorderPoint]
      );
    }
